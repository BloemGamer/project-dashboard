...
pub fn run(terminal: &mut DefaultTerminal, data: &mut Data, mut state: TasksState) -> tui::TuiState
{
    'tasks_render_loop: loop
    {
        ...
        match state
        {
            ...
            TasksState::Adding => 
            {
                terminal.draw(|frame| render_adding(frame, data)).unwrap();
            }
        };
        // input handeling
        if let Event::Key(key) = event::read().unwrap()
        {
            state = match state
            {
                ...
                TasksState::Main => 
                {
                    handle_keys_main(key, data)
                }
                TasksState::Adding => 
                {
                    handle_keys_adding(key, data)
                }
            }
        }
    }
...
fn handle_keys_main(key: KeyEvent, data: &mut Data) -> tui::TasksState
{
    match key.code
    {
        ...
        event::KeyCode::Char(char) =>
        {
            if let Some(tasks) = data.tasks.as_mut()
            {
                match char
                {
                    'A' => return tui::TasksState::Adding,
                    'X' =>
                    {
...
fn handle_keys_adding(key: KeyEvent, data: &mut Data) -> TasksState
{
    match key.code
    {
        event::KeyCode::Esc => { return tui::TasksState::Main; }
        event::KeyCode::Char(char) =>
        {
            if let Some(tasks) = data.tasks.as_mut()
            {
...
fn render_adding(frame: &mut Frame, data: &mut Data)
{
    Paragraph::new("Adding").render(frame.area(), frame.buffer_mut());
    let chunks: [Rect; 1] = Layout::default()
        .direction(Direction::Vertical)
        .margin(1)
        .constraints([Constraint::Fill(1)].as_ref())
